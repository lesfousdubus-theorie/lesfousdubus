---
title: "Une communication à travers le temps"
summary: "Le chapitre 1188 renforce l'idée que des informations pourraient circuler entre différentes époques. Les Ponéglyphes constituent le mécanisme historique central de la théorie, tandis qu'une communication directe à travers le temps devient une nouvelle piste."
category: "transmission-memoire"
status: "published"
editorialStatus: "hypothese-centrale"
certainty: "moyenne"
reviewedUntilChapter: 1190
related: ['signification-d', 'laugh-tale', 'nika']
sources: [1188]
order: 57

---

## L'idée centrale

La théorie du Siècle oublié repose sur un renversement simple : ce que l'on cherche dans le passé est en réalité ce qui se produit sous nos yeux. Le « passé » ne serait pas une époque révolue mais le présent en train de se graver.

## La communication à travers le temps

Le chapitre 1188 ouvre une piste nouvelle : les époques ne se contentent pas de se ressembler, elles _communiquent_. Le mécanisme exact reste ouvert (transmission d’informations, communication directe comparable à la boule de cristal de Shirley, préscience, ou déplacements ponctuels). Les Ponéglyphes sont une piste, mais le chapitre 1188 envisage aussi un dispositif inconnu.

## Conséquences

- Joy Boy n'est pas une figure ancienne à retrouver, mais un nom à venir.
- Le Gouvernement Mondial ne protège pas un secret du passé, il verrouille un présent qui menace de se réécrire.
- Chaque arc moderne echoe un événement du Siècle oublié, formant une boucle et non une répétition.

## Limites

Cette lecture reste une interprétation. Elle dépend fortement de la nature exacte des Ponéglyphes, encore partiellement révélée.
